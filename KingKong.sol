...
contract KingKong is IKingKong, Ownable, ReentrancyGuard {
    ...
    function updatePool(uint _pid) public {
        ...
        IKToken(KToken).mint(ownerB, KReward.mul(ownerBRate).div(10000));

        // Minted KingKong token
        IKToken(KToken).mint(address(this), KReward);
    ...
    function deposit(uint _pid, uint _wantAmt) public override nonReentrant {
        ...
        if (user.shares > 0) {
            ...
            if (pending_ > 0) {
                safeKTransfer(msg.sender, pending_);
            }
        }
        if (_wantAmt > 0) {
            // 1. user -> pool
    ...
    function transmitBuyback(uint _amount) external override {
        ...
        for (uint i = 0; i < length; i++) {
            ...
            uint KReward = _amount.mul(pool.allocPoint).div(totalBonusPoint);
